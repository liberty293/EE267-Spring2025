...
void main() {
	...
	for(int i = 0; i < NUM_POINT_LIGHTS; i++) {
		...
		vec4 L = Lview - P; //vector from x to light position in view space
	...
	for(int i = 0; i < NUM_DIR_LIGHTS; i++) {
		vec4 Lview = viewMat* vec4(directionalLights[i].direction,0.0);
		vec3 R = reflect(normalize(Lview.xyz),N);
		float d = max(dot(N,-normalize(Lview.xyz)),0.0);
		float s = pow(max(dot(R,normalize(-P.xyz)),0.0),material.shininess);
		fColor += (d * material.diffuse * directionalLights[i].color + s * material.specular * directionalLights[i].color);
